...
def generate_with_hf_openai_api(prompt: str, max_tokens: int = None):
    """Generate text using HuggingFace OpenAI-compatible API (v1/chat/completions)."""
    try:
        import requests
    except ImportError:
...
def generate_with_inference_api(prompt: str, max_tokens: int = None):
    """Generate text using HuggingFace Inference API (deprecated - use OpenAI API instead)."""
    try:
        import requests
    except ImportError:
        logger.error("❌ requests not installed")
    ...
    try:
        logger.info(f"🤖 Using HF Inference API with model {cfg.MODEL_ID}")
        
        api_url = f"https://router.huggingface.co/hf-inference/models/{cfg.MODEL_ID}"
        headers = {"Authorization": f"Bearer {token}"}
...
def generate_local(prompt: str, max_tokens: int = None):
    """Generate text using local transformer model (optimized for M4 MacBook)."""
    try:
...
def generate(prompt: str, max_tokens: int = None):
    """Generate newsletter content using configured method."""
    if not prompt or not prompt.strip():
        raise ValueError("❌ Prompt cannot be empty")
    
    try:
        if cfg.USE_HF_INF_API:
            # Try the new OpenAI-compatible API first
